...
dir_before_QC = config::get("dir_before_QC")
nCount_ATAC_min = config::get("nCount_ATAC_min")
nCount_ATAC_max = config::get("nCount_ATAC_max")
nCount_RNA_min = config::get("nCount_RNA_min")
nCount_RNA_max = config::get("nCount_RNA_max")
MT = config::get("MT")
nucleosomesignal = config::get("nucleosomesignal")
TSS_enrichment = config::get("TSS_enrichment")
qc_data = config::get("qc_data")
...
```{r,echo=F,message=F,warning=F}
SSc.gene.multiome.seurat <- readRDS(dir_before_QC)

DefaultAssay(SSc.gene.multiome.seurat) <- "ATAC"

SSc.gene.multiome.seurat <- NucleosomeSignal(SSc.gene.multiome.seurat)
SSc.gene.multiome.seurat <- TSSEnrichment(SSc.gene.multiome.seurat,fast=FALSE)


SSc.gene.multiome.seurat$high.tss <- ifelse(SSc.gene.multiome.seurat$TSS.enrichment > TSS_enrichment, paste('TSS>',TSS_enrichment,sep=""),paste('TSS<=',TSS_enrichment,sep=""))

SSc.gene.multiome.seurat$nucleosome_group <- ifelse(SSc.gene.multiome.seurat$nucleosome_signal > nucleosomesignal, paste('NS > ', nucleosomesignal,sep=""), paste('NS < ', nucleosomesignal,sep=""))
...
Idents(SSc.gene.multiome.seurat) <- SSc.gene.multiome.seurat$sample

sample_name = unique(SSc.gene.multiome.seurat@meta.data[["sample"]])

for (i in (1:length(sample_name))){

  temp.sample <- subset(SSc.gene.multiome.seurat, idents = i)
...
if (qc_data){
  ...
  ref2 <- as.data.frame(colnames(SSc.gene.multiome.seurat))
  colnames(ref2) <- "barcodes.check"
  ref <- merge(x = ref, y = ref2, by = "barcodes.check")

#check the order of barcodes
#all(colnames(SSc.gene.multiome.seurat)==ref$barcodes.check)

  SSc.gene.multiome.seurat$QC <- ref$temp.QC

  SSc.gene.multiome.seurat.filtered <- subset(x = SSc.gene.multiome.seurat,  subset = QC==TRUE)
...
print(paste("After Quality control the number of cells:", as.numeric(ncol(SSc.gene.multiome.seurat.filtered)),"unique RNA reads:",as.integer(sum(SSc.gene.multiome.seurat.filtered$nCount_RNA))))


print(paste("After Quality control the number of cells:", as.numeric(ncol(SSc.gene.multiome.seurat.filtered)),"unique ATAC reads:",as.integer(sum(SSc.gene.multiome.seurat.filtered$nCount_ATAC))))
```
...
Idents(SSc.gene.multiome.seurat.filtered) <- SSc.gene.multiome.seurat.filtered$sample
n.cell<-c()
n.reads_RNA<-c()
n.reads_ATAC<-c()
sample_id<-c()
...
for (i in (1:length(sample_name))){
  ...
  tmp<-subset(SSc.gene.multiome.seurat.filtered, idents=i)
...
for (i in (1:length(sample_name))){
  ...
  temp.sample<-subset(SSc.gene.multiome.seurat.filtered,idents=i)
...
print(VlnPlot(SSc.gene.multiome.seurat.filtered, features = c("nCount_ATAC", "nCount_RNA","percent.mt", "TSS.enrichment", "nucleosome_signal"), ncol = 3, log = TRUE, pt.size = 0) + NoLegend())

saveRDS(SSc.gene.multiome.seurat.filtered,file=paste(output_dir,'/','after_QC.RDS', sep=""))
```
