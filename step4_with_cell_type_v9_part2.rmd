...
dir_final = config::get("dir_final")
...
```{r,echo=F,message=F,warning=F}
SSc.gene.multiome.seurat.filtered<-readRDS(dir_final)
...
eval(parse(text = paste(
    "SSc.gene.multiome.seurat.filtered$cell.type_",
    VOI,
    " <- ",
    "paste(SSc.gene.multiome.seurat.filtered$",
    VOI,
    ",SSc.gene.multiome.seurat.filtered$cell.type,sep='__')",
    sep = ""
  )
))

eval(parse(text = paste(
    "Idents(SSc.gene.multiome.seurat.filtered) <- SSc.gene.multiome.seurat.filtered$cell.type_",
    VOI,
    sep = ""
  )
))
...
for (j in DE.Args) {
  ...
  if (eval(parse(
    text = paste(
      "(length(which(SSc.gene.multiome.seurat.filtered$cell.type_",
      VOI,
      " == ident1)) > 3) &
    (length(which(SSc.gene.multiome.seurat.filtered$cell.type_",
    VOI,
    ...
    sub1 <- subset(SSc.gene.multiome.seurat.filtered, idents = ident1)
    sub2 <-
      subset(SSc.gene.multiome.seurat.filtered, idents = ident2)
    adjust <- c()
